...
def _extract_normalized(data: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in _as_list(data.get("permissions", [])):
        if isinstance(item, str):
            ...
            permissions.append({"action": str(item["action"]), "resource": str(item.get("resource", "*")), "effect": str(item.get("effect", "Allow"))})
    return permissions
...
def _extract_gcp(data: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for binding in _as_list(data.get("bindings")):
        ...
        permissions.append({"action": f"role:{role}", "resource": "*", "effect": "Allow", "kind": "gcp-role", "members": _as_list(binding.get("members"))})
    return permissions
...
def _evidence_from_root(data: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "failure_injection": data.get("failure_injection") or data.get("failureInjection"),
    }
...
def _usage_calls(usage: Any, action: str) -> int | None:
    ...
    calls = item.get("calls", item.get("count")) if isinstance(item, dict) else item
    if calls is None:
        return None
    try:
        return int(calls)
...
def _dependency_permissions(dependencies: list[Any]) -> set[str]:
    ...
    for dep in dependencies:
        ...
        for key in ("permission", "action", "requires", "required_permission"):
            for item in _as_list(dep.get(key)):
...
def analyze_import(data: Any, filename: str) -> dict[str, Any]:
    ...
    if provider == "aws":
        permissions, sources = _extract_aws(source), ["AWS IAM policy document"]
    elif provider == "azure":
        permissions, sources = _extract_azure(source), ["Azure RBAC role assignment export"]
    elif provider == "gcp":
        permissions, sources = _extract_gcp(source), ["Google Cloud IAM policy binding export"]
    elif provider == "normalized":
        permissions, sources = _extract_normalized(source), ["AegisIAM normalized policy"]
        root_evidence = {**root_evidence, **_evidence_from_root(source)}
    else:
        permissions, sources = [], ["Generic JSON import"]
        if isinstance(source, dict):
            permissions = [{"action": str(a), "resource": "*", "effect": "Allow"} for a in _as_list(source.get("actions"))]
    if not permissions:
    ...
    for p in permissions:
        ...
        decisions.append({"permission": action, "resource": resource, "decision": decision, "reason": reason, "sensitive": sensitive, "calls": calls, "dependency": dependency, "evidence": "IAM + audit + dependency" if calls is not None and dependency else ("IAM + audit" if calls is not None else "IAM policy only")})
    ...
    initial_removed = [p for p in original_policy if p not in proposed]
    ...
    events = [
        ...
        {"agent": "Policy Strategist", "role": "Generate evidence-backed least-privilege recommendations", "message": f"Generated {len(initial_removed)} removal candidate(s).", "duration_ms": 0},
    ]
    ...
    if services:
        ...
        cloud = SyntheticCloud({"id": "imported-evidence", "name": f"Imported IAM · {filename}", "role": role, "permissions": original_policy, "services": services, "dependencies": dependencies})
        ...
        if simulation["status"] == "FAIL":
            restored = sorted({p for result in simulation["services"] if result["status"] == "FAIL" for p in result["missing_permissions"]})
            proposed = list(dict.fromkeys(proposed + restored))
            replanned = True
            events.extend([
                {"agent": "Root Cause Agent", "role": "Diagnose failed service and missing permissions", "message": f"Detected {len(restored)} missing permission(s) required by failed service(s).", "duration_ms": 0},
                {"agent": "Replanner", "role": "Restore only permissions proven necessary for functionality", "message": f"Restored {len(restored)} dependency permission(s) and re-simulated.", "duration_ms": 0},
            ])
            cloud.set_policy(proposed)
            simulation = cloud.simulate()
    ...
    if services:
        service_results = simulation.get("services", [])
        services_healthy = all(x["status"] == "PASS" for x in service_results if any(s.get("id") == x.get("id") and s.get("critical", False) for s in services))
        if not any(s.get("critical", False) for s in services):
            services_healthy = all(x["status"] == "PASS" for x in service_results)
        functionality_score = round(100 * sum(x["status"] == "PASS" for x in service_results) / max(1, len(service_results)))
    ...
    return {
        "run_id": f"import-{filename}-{len(permissions)}", "status": "ANALYSIS_COMPLETE" if final_status == "ANALYSIS_ONLY" else final_status,
        "mode": "IMPORTED_IAM", "source_file": filename, "provider": provider.upper(),
        ...
        "decisions": decisions, "proposed_policy": list(dict.fromkeys(proposed)), "simulation": simulation, "events": events,
        "red_team_findings": red_team, "replans": [{"reason": "Service failure after least-privilege proposal", "restored": restored}] if replanned else [],
        "verification": {"status": final_status, "security_score": security_score, "functionality_score": functionality_score, "removed": removed, "red_team_clean": not red_team, "services_healthy": services_healthy, "evidence_coverage": "GOOD" if evidence_complete else "PARTIAL", "note": "Evidence-enriched imports can be simulated in the local sandbox. IAM-only exports remain analysis-only because usage and dependencies cannot be inferred safely."},
        "import_summary": {"permission_count": len(permissions), "removed_candidates": len(removed), "high_risk_permissions": sum(1 for d in decisions if d["sensitive"]), "investigate_permissions": investigate_count, "evidence_sources": sources, "dependencies_supplied": len(dependencies), "services_supplied": len(services), "audit_entries_supplied": sum(1 for d in decisions if d["calls"] is not None)},
    }
